...
#               "a real-number square can never be negative, so a new number was invented"
#               (i is not an exception, it is not real); the discriminant's sign "says how
#               the curve meets the x line: two crossings, one touch, or none". The condition
#               class, ten: degrees add IN THAT TIMESING; factors OF THE FORM x take away a
#               number; roots under one roof FOR NUMBERS THAT ARE NOT NEGATIVE; exponents add
#               for powers OF THE SAME BASE; logs add for POSITIVE numbers in ONE LOG BASE;
#               power questions turn around FOR THE POSITIVE BASES WE USE; slots times when
#               every choice fits EVERY COMBINATION SO FAR (why and recap); the mirror is 360
#               MINUS THE ANGLE, not one more turn; the wiggle count is a rule for now, its
#               proof waits for calculus; the log of 18 nearness is an estimate, said so.
#               Words and board (the xs class, here): 3 - 8 = -5, 4 - 9 = -5, 5 - 11 = -6 drawn
#               before the bars drop the sign; the line x = 4 drawn on the forbidden-x graph;
#               the test-number formula on its recap board; 20 x 20 = 400 drawn; x = ±3i on the
#               recap caption; the bananas reason question reads its two trips; "the x sits
#               after the divide sign"; "the vertex lesson's flip does not belong here";
#               "squaring wipes a minus, so both give negative 9". Three sentences split for
#               length. Declined: the reason choices are never read aloud (tap buttons by
#               design; a charter line in coursesweep.py now). 28 edits; no beat added, the
#               course list stays 40,495. PART 3nq.
...
#               reading: (1) every closing recap board whose equation the words did not read is read now -- 24 lessons; 
#               (2) every sentence of 27 words or more in a why, picture, teach, worked or
#               recap beat is split -- 35 sentences. Numbers said in words ("four plus four
...
#               (three-crossings); the test number's recap says which curves; the
#               reciprocal's recap says positive x's and never zero; the forbidden x's recap
#               says a bottom like x − a; the survivor's recap says the shape (a x + b over
#               x); "in general, two roots go under one roof" -> for numbers that are not
#               negative; the one-half power is the POSITIVE number that squares to the base;
#               logs add "for numbers in the same base" and it is "a great law", not the one;
#               between-the-powers' nearest-neighbour rule is "for today's numbers ... a
#               guide, not a law" (the true halfway point on the log scale sits below the
#               middle); decay's halving is "in these problems"; a polynomial "of degree 1 or
#               higher" turns at most one fewer time; the counting principle needs every
#               choice to go with every choice; expected value is a long-run average, "about
#               2 times out of 6", "a game like these"; a FAIR sample speaks for the school;
#               "every function question can turn around" -> many. TRUE STATEMENTS: |−5| is 5,
#               not "neither is negative 5" (inside-the-distance); i's lesson says both
#               answers, 3i and −3i, and takes the positive one (picture and recap); "cubed
#               means times itself three times" -> three copies timesed together, x·x·x; the
#               half turn flips SINE AND COSINE, and 360 − θ is the mirror angle, not a full
#               turn added; adding polynomials USUALLY lets the biggest survive; roots don't
#               add: √3 + √48 is not √51 and not 51; "the reciprocal's strange charm" is gone
#               (the x sits after the divide sign). WORDS-BOARD: the wide-fence caption says
#               the line is compressed and which dots are drawn; the three-friends reason
#               draws the pair clues; walk-the-rule's picture draws the SECOND pass
#               (9 -> 17) the words describe and its trap board says 20 (5 -> 10 -> 20), not
#               18; the hidden exponent's why uses 81 forwards AND backwards (3^? = 81,
#               ? = 4, four threes); 1-to-13 is paired by the rectangle (13 rows of 14 hold
#               two copies), not "every pair is 14"; pair-the-ends' pairs are written as
#               sums with "every pair is 11"; wiggle-count's board writes degree 6 -> at most
#               5 beside the spoken one; degrees-add's why says "x cubed times x squared is x
#               to the fifth" over its board; logs-add's second worked line says 64 × 128 is
#               8192; seven closing beats speak the equation their board writes. THE DOT: 17
#               authored step lines that joined two equations with " · " are two [[step]]
#               tags each (three captions say "and"); x³ · x² = x⁵ and √a · √b (real
#               products) stay. Two reviewer objections became charter lines instead
#               (coursesweep.py): a ✗ marks the wrong path, and a bare "log" on a board is
#               base 2. Generator items: lessonscripts.py (wm).
...
_ALGEBRA2_U1 = [
    {
        ...
        "teach": [
            ("The same bars measure the gap between two numbers. Take 8 away from 3 "
             ...
             "leave the 5.",
             '[[numberline min="1" max="10" points="3,8" hops="3,8" caption="3 and 8 — 5 steps apart"]][[step eq="3 − 8 = −5"]][[step eq="|3 − 8| → 5 apart"]]'),
        ...
        "pairs": [
            {"worked": ("Here is one more, done for you. Take 9 away from 4 and you get "
                        ...
                        "apart.",
                        '[[numberline min="2" max="11" points="4,9" hops="4,9" caption="4 and 9 — 5 steps apart"]][[step eq="4 − 9 = −5"]][[step eq="|4 − 9| → 5"]]'),
             "ask": {'a': 2, 'b': 5, 'op': 'absv'}},
            {"worked": ("One more together. Take 11 away from 5 and you get negative 6. "
                        "Drop the sign: the absolute value is 6.",
                        '[[numberline min="3" max="13" points="5,11" hops="5,11" caption="5 and 11 — 6 steps apart"]][[step eq="5 − 11 = −6"]][[step eq="|5 − 11| → 6"]]'),
        ...
        "recap": [
            ...
            ("And that is two straight bars, and the one question they ask: how far is 3 from 8? 5.",
             '[[step eq="|3 − 8| = 5"]]'),
        ],
        "bank": [
            {"a": 3, "b": 5, "op": "absv"},
    ...
    },
    {
        ...
        "picture": [
            ("Here is the number line with the fence at negative 5 and 5. Every dot is "
             ...
             "posts themselves stay out: 5 is not LESS than 5, and negative 5 sits 5 "
             "steps from zero — not less than 5 either.",
        ...
        "pairs": [
            {"worked": ("Here is one more, done for you — a smaller fence, less than 2. "
             ...
             "ask": {'a': 3, 'b': 0, 'op': 'absc'}},
            {"worked": ("One more together — a wide fence, less than 20. The board draws "
                        "only the outermost dots, negative 19 and 19, and zero. Left of "
                        "zero: negative 19 up to negative 1, nineteen integers. Right of "
                        "zero: 1 up to 19, nineteen more. And zero. Nineteen plus nineteen "
                        "plus one is 39.",
                        '[[numberline min="-20" max="20" points="-19,0,19" caption="the line is compressed: only −19, 0 and 19 are drawn — 19 integers each side of zero; the posts at ±20 stay out"]][[step eq="19 + 19 + 1 = 39"]]'),
        ...
        "recap": [
            ...
            ("And that is a distance, counted from the inside: 4 plus 4 plus 1 is 9.",
             '[[step eq="4 + 4 + 1 = 9"]]'),
        ],
        "bank": [
            {"a": 4, "b": 0, "op": "absc"},
    ...
    },
    {
        ...
        "teach": [
            ...
            ("The trap is stopping early. 6 is real — it is what TWO apples cost — but "
             ...
             "share.",
             '[[step eq="2 apples = 6"]][[step eq="1 apple = 3 ✓"]][[step eq="6 ✗ — that is the pair, not the apple"]]'),
        ],
        "pairs": [
            {"worked": ("Here is one more, done for you. Three apples and two bananas: 22 "
                        ...
                        '[[tape parts="apple | apple" total="14" caption="2 apples = 22 − 8 = 14 — one apple is 7"]][[step eq="2 apples = 22 − 8 = 14"]][[step eq="1 apple = 7"]]'),
             "ask": {'a': 12, 'b': 6, 'op': 'el2'}},
            {"worked": ("One more together. Trips of 19 and 9: the bananas cancel, 2 "
                        "apples cost 10 — one apple is 5 cents.",
                        '[[tape parts="apple | apple" total="10" caption="2 apples = 19 − 9 = 10 — one apple is 5"]][[step eq="2 apples = 10"]][[step eq="1 apple = 5"]]'),
             "ask": {'a': 25, 'b': 9, 'op': 'el2'}},
        ],
        "practice_intro": ("Now it's your turn. Three right answers in a row and we're "
                           "done — here comes the first one."),
        "show_work_on_correct": True,
        "explain": {
            "spoken": ("One more thing — not the answer, the reason. Three apples and two bananas cost 14 cents; one apple and the same two bananas cost 8. One apple costs 3. Tap the reason why."),
        ...
        "recap": [
            ...
            ("And that is Algebra One\'s trick, with one more step.",
             '[[step eq="2 apples = 6"]][[step eq="1 apple = 3"]]'),
        ],
        "bank": [
            {"a": 10, "b": 6, "op": "el2"},
            {"a": 13, "b": 7, "op": "el2"},
    ...
    },
    {
        ...
        "picture": [
            ("Here are the three clues as bars: the first pair weighs 7, the second "
             "10, the third 9. Look at who is inside each bar. x is in the first and the third, y in the first and the second, z in the second and the third. "
        ...
        "explain": {
            ...
            "board": '[[bars data="x + y:7 | y + z:10 | x + z:9 | all three clues:26 | everyone once:13" caption="7 + 10 + 9 = 26 — everyone twice — 26 ÷ 2 = 13"]]',
        },
        "recap": [
...
_ALGEBRA2_U2 = [
    {
        ...
        "recap": [
            ("So, here it is again. A curve written as x take away a number, squared, turns where that squared part is zero. That is at the number itself, with the opposite sign to the one you see. The plus at the end says how high; the "
             "take away inside says where.",
             '[[graph func="(x-3)^2+2" points="(3,2)" range="-1..7" caption="where: x = 3 — how high: 2"]]'),
            ("And that is the vertex's x, read straight off the formula: x take away 3 is 0, so x is 3.",
    ...
    },
    {
        ...
        "recap": [
            ("So, here it is again. In these examples, each factor donates one root, "
             "one crossing of the ground. When the question asks for the sum, it means "
             "both of them — add the two roots, and never mistake their product for "
             "their sum.",
             '[[graph func="(x-2)*(x-5)" points="(2,0),(5,0)" range="-1..8" caption="two roots, both counted"]]'),
            ("And that is the first secret a pair of roots shares: 2 plus 5 is 7.",
             '[[step eq="2 + 5 = 7"]]'),
        ],
        "bank": [
            {"a": 2, "b": 3, "op": "rsum"},
    ...
    },
    {
        ...
        "why": [
            ("Why a test number? Because you can count a curve\'s crossings without "
             "drawing it. y equals x squared plus 2 x plus 7 hides a test number, called the discriminant. Square the number in front of x, then take away 4 times the plain number. Its SIGN — not its size — says how the curve meets the x line: two crossings, one touch, or none.",
             '[[goal text="The test number"]][[step eq="x² + a·x + b → test: a² − 4b"]]'),
        ],
        "picture": [
            ("Here are the two numbers the test compares, as bars: 2 squared, which is "
             "4, against 4 times 7, which is 28. The second bar is taller, so the take away falls below zero. And here is the curve itself, floating above the x line, never touching it.",
        ...
        "recap": [
            ("So, here it is again. For today's curves — x squared, plus some x's, plus a plain number — the test number is this. The number in front of x, squared, take away 4 times the plain number. Only its sign matters: positive, two "
             "crossings; zero, one touch; negative, none. Report the meetings with the x line, never "
             "the messenger.",
             '[[step eq="x² + a·x + b → test: a² − 4b"]][[graph func="x^2+2*x+7" range="-5..3" caption="a negative test — the curve never comes down"]]'),
            ("And that is a curve counted without being drawn: below zero, no crossings; zero, one; above zero, two.",
    ...
    },
    {
        ...
        "picture": [
            ("Here is 9 as a square: 3 rows of 3. That is the everyday half "
             ...
             "i\'s job: one answer is 3 times i, and the other is negative 3 times i. Squaring wipes a minus, so both give negative 9. Today we take the one with the positive number in front of i.",
        ...
        "teach": [
            ("That is the method. With i in hand, x equals 3 i solves it. Check: 3 i "
             ...
             "equals negative 9. A real number times i, like 3 i, is called an imaginary number, though it is as real to mathematics as any invention that works.",
             '[[array rows="3" cols="3" caption="(3i)² = 9 × i² = −9"]][[step eq="(3i)² = 9 × i²"]][[step eq="9 × (−1) = −9 ✓"]]'),
            ("Handle it with care. x squared equals negative 9 does not mean x is "
             ...
             '[[array rows="3" cols="3" caption="(−3)² is +9 — the wrong sign; the minus is i\'s job"]][[step eq="x = 3i ✓ — the i carries the minus"]][[step eq="−3 ✗ (−3)² = +9"]][[step eq="9 ✗ forgot the root"]]'),
        ],
        "pairs": [
        ...
        "recap": [
            ("So, here it is again. A real-number square can never be negative, so a new number was invented: i, whose one job is i squared equals negative 1. Then x squared equals negative a, for a positive a, is solved by plus or minus the root of a, times i. Today we report the one with the positive number in front. The i carries the minus; the number "
             "carries the root.",
             '[[array rows="3" cols="3" caption="x² = −9 → x = ±3i — today, 3i"]]'),
            ("And that is the door the unit opens: i squared is negative 1, and x is 3 i.",
             '[[step eq="i² = −1"]][[step eq="x = 3i"]]'),
        ],
        "bank": [
            {"a": 36, "b": 0, "op": "imag"},
            {"a": 49, "b": 0, "op": "imag"},
...
_ALGEBRA2_U3 = [
    {
        ...
        "why": [
            ("Why degrees? Polynomials are algebra\'s long expressions, and their "
             ...
             "counts. Degrees ride the same rule: x cubed times x squared is x to the "
             "fifth.",
             '[[goal text="Degrees add"]][[step eq="x³ · x² = x⁵"]]'),
        ],
        "picture": [
            ("Here are the two top powers as piles of x's. x to the 4 is a pile of four, and x cubed is a pile of three. Times them and the piles join into "
        ...
        "recap": [
            ("So, here it is again. A polynomial\'s degree is its highest power, and "
             ...
             "add their counts. In that timesing the degrees add; they never times, and the bigger one never simply survives.",
    ...
    },
    {
        ...
        "teach": [
            ("That is the method. Why one fewer? The full proof waits for calculus; for now the picture is the rule. Between two turns the curve must climb or fall, and the last stretch runs off to the horizon without turning back. So degree 4: at most 3 turns. Degree 6: at most 5. The wiggles can "
             "be fewer — they can flatten away — but never more.",
             '[[graph func="x^4-4*x^2" range="-3..3" caption="degree 4 → at most 3 turns"]][[step eq="degree 4 → at most 3 turns"]][[step eq="degree 6 → at most 5 turns"]]'),
        ...
        "recap": [
            ("So, here it is again. A polynomial of degree 1 or higher can turn at most one fewer time than that degree. It is a rule for these graphs that Calculus will explain. At most — a ceiling, not a schedule.",
             '[[graph func="x^4-4*x^2" range="-3..3" caption="one fewer turn than the degree"]]'),
            ("And that is a picture promised by a number: degree 4, at most 3 turns.",
    ...
    },
    {
        ...
        "why": [
            ("Why three crossings? Algebra One factored quadratics; Algebra Two grows "
             "them. x take away 1, times x take away 3, times x take away 5, equals zero. THREE factors, a cubic, three crossings: 1, 3 and 5. Each different "
        ...
        "recap": [
            ("So, here it is again. Every different factor of the form x take away a number donates one root, so three different ones make three crossings. A question about the answers means all of them — "
            ...
            ("And that is a quadratic's trick, grown to a cubic: 1 plus 3 plus 5 is 9.",
             '[[step eq="1 + 3 + 5 = 9"]]'),
        ],
        "bank": [
    ...
    },
    {
        ...
        "picture": [
            ("Here is the machine with its rule written on it: x cubed, take away 2 x, "
             "plus 3. Feed in x equals 5 and the machine works the whole recipe on that 5 — cubes it, takes away twice it, adds 3. Out comes 118.",
        ...
        "recap": [
            ("So, here it is again. A polynomial is a machine: feed the x into every "
             "place it appears. Read each power as a power — cubed means three copies timesed together, x times x times x, never 3 times x. And carry every sign with you.",
             '[[machine input="5" rule="x³ − 2x + 3" output="118" caption="read the power, keep the sign"]]'),
            ("And that is a function fed, one level taller: 5 cubed, take away 2 times 5, plus 3, is 118.",
             '[[step eq="5³ − 2·5 + 3 = 118"]]'),
        ],
        "bank": [
            {"a": 3, "b": 2, "c": 2, "op": "pval"},
...
_ALGEBRA2_U4 = [
    {
        ...
        "teach": [
            ...
            ("The traps are the other operations wearing masks. 14 divided by 2 is 7 — "
             ...
             '[[step eq="14 ÷ 2 = 7 ✓"]][[step eq="14 − 2 = 12 ✗ a mask"]][[step eq="14 × 2 = 28 ✗ a mask"]]'),
        ],
        "pairs": [
        ...
        "recap": [
            ("So, here it is again. For the positive x\'s we used, y equals a number "
             "divided by x shares that number among x pieces. The bigger the crowd, "
             "the smaller each share — a curve that falls and flattens, and x itself "
             "is never zero. The word is divided: not take away, not times.",
             '[[graph func="14/x" points="(2,7)" range="0..15" caption="sharing shrinks"]]'),
            ("And that is growth, run backwards: 14 shared by 2 is 7.",
             '[[step eq="14 ÷ 2 = 7"]]'),
        ],
        "bank": [
            {"a": 6, "b": 2, "op": "rdiv"},
    ...
    },
    {
        ...
        "why": [
            ("Why run it backwards? Because many function questions can turn around. "
        ...
        "recap": [
            ...
            ("And that is how to find the x under a divide sign. When a number divided by x equals an answer, divide the number by the answer: x is 20 divided by 5, 4.",
             '[[step eq="x = 20 ÷ 5 = 4"]]'),
        ],
        "bank": [
            {"a": 8, "b": 2, "op": "rsol"},
    ...
    },
    {
        ...
        "why": [
            ("Why forbidden? Because every function so far accepted every x, and "
             "division ends that. y equals 5 divided by: x take away 4. Feed most x's and all is well — but ONE x breaks the machine, because it turns the bottom into zero. Dividing by zero is the one thing mathematics never allows. That x is forbidden.",
        ...
        "picture": [
            ("Here is the curve of 5 divided by x take away 4. Look at x equals 4: the "
             ...
             '[[graph func="5/(x-4)" lines="x=4" range="0..8" yrange="-12..12" caption="y = 5 ÷ (x − 4) — the curve flies off at the line x = 4 and never lands"]]'),
        ],
        "teach": [
        ...
        "explain": {
            "spoken": ("One more thing — not the answer, the reason. y equals 5 divided "
                       "by the quantity x take away 4, and the forbidden x is 4. Tap the "
                       "reason why."),
        ...
        "recap": [
            ("So, here it is again. For today\'s functions, with a bottom like x take "
             "away a number, the one forbidden x is the one that turns the bottom into "
             "zero. Find it by asking when the bottom dies. Not the sign flipped, and "
             "not zero by habit: the danger is where the bottom is zero, not where x "
             "is.",
             '[[graph func="5/(x-4)" range="0..8" yrange="-12..12" caption="one forbidden x on an endless road"]]'),
            ("And that is an x this function refuses: x take away 4 is 0, so x equals 4 is forbidden.",
    ...
    },
    {
        ...
        "recap": [
            ...
            ("And that is a function, read at the horizon: y equals 2 plus 6 over x settles at 2.",
...
_ALGEBRA2_U5 = [
    {
        ...
        "why": [
            ("Why one roof? Because roots come back with a rule that turns ragged "
             ...
             "root of 2 times the root of 2 is exactly 2. For numbers that are not negative, two roots can go under one roof. Root of a, times root of b, equals the root of a times b.",
             '[[goal text="Under one roof"]][[step eq="√a · √b = √(a·b)"]]'),
        ],
        "picture": [
            ("Here is the square that the two roots make together. The square root of 3 times the square root of 48 is the root of 3 times 48 — the root of 144. And 144 is a perfect square: 12 rows of 12. The side of that square, "
             "12, is the answer.",
        ...
        "recap": [
            ("So, here it is again. For numbers that are not negative, two square roots timesed go under one roof: the root of the product. If that product is a perfect square, two ragged roots "
    ...
    },
    {
        ...
        "why": [
            ("Why a fraction in the exponent? Because exponents can wear fractions. "
             ...
             "the one-half power is the positive number that times ITSELF into 25. That "
             "is a square root.",
        ...
        "recap": [
            ("So, here it is again. A one-half power is the positive number that times "
             "itself into the base — the square root. The fraction is a costume the root "
             "wears; it is never an instruction to halve.",
             '[[bars data="√25 = 5:5 | half of 25:12.5" caption="a root, never a halving"]]'),
            ("And that is an exponent that unbuilds a square: 25 to the power one half is the root of 25, 5.",
             '[[step eq="25^½ = √25 = 5"]]'),
        ],
        "bank": [
            {"a": 36, "b": 0, "op": "rpow"},
    ...
    },
    {
        ...
        "pairs": [
            {"worked": ("Here is one more, done for you. Root of 90: between 81 and 100, so "
                        ...
                        "to 9.",
                        '[[numberline min="81" max="100" points="90" hops="81,90" caption="90 − 81 = 9 and 100 − 90 = 10 — nearer 81, so √90 → 9"]][[step eq="81 < 90 < 100 → √90 → 9"]]'),
             "ask": {'a': 20, 'b': 0, 'op': 'rbet'}},
            {"worked": ("One more together. Root of 30: between 25 and 36, and 30 leans "
                        "toward 25 — closest to 5.",
                        '[[numberline min="25" max="36" points="30" hops="25,30" caption="30 − 25 = 5 and 36 − 30 = 6 — nearer 25, so √30 → 5"]][[step eq="25 < 30 < 36 → √30 → 5"]]'),
        ...
        "recap": [
            ...
            ("And that is a ragged root — one that is not whole — placed: root 40 sits between 6 and 7, closest to 6.",
...
_ALGEBRA2_U6 = [
    {
        ...
        "teach": [
            ...
            ("The trap is the linear faller — the doubling pond\'s old enemy, walking "
             ...
             '[[step eq="48 ÷ 2 ÷ 2 ÷ 2 = 6 ✓"]][[step eq="48 − 6 = 42 ✗ the linear faller"]][[step eq="24 ✗ one day only"]]'),
        ],
        "pairs": [
        ...
        "recap": [
            ("So, here it is again. Decay is fading by times. In these problems, each day the sample drops to half of whatever it was. A divide by 2, once for every day, never a take away. Keep dividing until the days run out.",
             '[[bars data="day 0:48 | day 1:24 | day 2:12 | day 3:6" caption="a divide each day"]]'),
            ("And that is the doubling pond\'s shadow: 48 divided by 2, by 2, by 2 is 6.",
             '[[step eq="48 ÷ 2 ÷ 2 ÷ 2 = 6"]]'),
        ],
        "bank": [
            {"a": 16, "b": 2, "op": "hlfl"},
    ...
    },
    {
        ...
        "why": [
            ("Why hidden? Because, for the positive bases we use, every power question can turn around. Forward: 3 to "
             "the power 4 is 81. Backwards: 3 raised to WHAT equals 81? Count the "
             "threes: 3 times 3 times 3 times 3 — four of them. That backwards question "
             "is one of the great tools of mathematics, and its answer has a name: the "
             "logarithm.",
             '[[goal text="The hidden exponent"]][[step eq="3^? = 81"]][[step eq="? = 4"]]'),
        ],
        "picture": [
        ...
        "recap": [
            ...
            ("And that is the hidden exponent, found: 3 cubed is 27, so the log is 3.",
    ...
    },
    {
        ...
        "why": [
            ("Why do logs add? Because the logarithm has a great law, and you have met its family twice. Exponents add when powers of the same base times, and degrees add when polynomials times. Now the logarithm — which IS a hidden exponent — obeys the same music. For positive numbers, in one log base, the log of a product is the logs, put together.",
             '[[goal text="Logs add"]][[step eq="log(a · b) = log a + log b"]]'),
        ],
        "picture": [
            ("Here are two stacks of doublings as bars, base 2. The log of 2 is 1 — one doubling — and the log of 16 is 4 — four doublings. Join the two "
        ...
        "teach": [
            ...
            ("Two traps. Multiplying the logs — 1 times 4 — treats the counts like "
             ...
             '[[step eq="log(2 · 16) = 1 + 4 = 5 ✓"]][[step eq="1 × 4 = 4 ✗ counts add"]][[step eq="2 + 16 = 18 ✗ wrong world"]]'),
        ],
        "pairs": [
            ...
            {"worked": ("One more together. Log of 64 is 6, log of 128 is 7; 64 times 128 "
                        "is 8192, and the log of 8192 is 6 plus 7 — 13.",
        ...
        "recap": [
            ...
            ("And that is one of the logarithm's great laws. For a and b above zero, in one base, the log of a times b is log a plus log b.",
    ...
    },
    {
        ...
        "why": [
            ("Why between the powers? Because most numbers are not perfect powers — "
             ...
             "between the squares. Take the logarithm, base 2, of 18. 18 is not a power of 2, but it sits between 16 and 32 — between 2 to the 4 and 2 to the 5. "
        ...
        "pairs": [
            {"worked": ("Here is one more, done for you. Log base 2 of 40: between 32 and "
                        ...
                        '[[bars data="2⁵:32 | 40:40 | 2⁶:64" caption="40 − 32 = 8 and 64 − 40 = 24 — nearer 32, so log 40 → 5"]][[step eq="32 < 40 < 64 → log 40 → 5"]]'),
             "ask": {'a': 17, 'b': 0, 'op': 'lbet'}},
            {"worked": ("One more together. Log base 2 of 26: it sits 10 past 16 and only "
                        "6 short of 32 — closest to 5.",
                        '[[bars data="2⁴:16 | 26:26 | 2⁵:32" caption="26 − 16 = 10 and 32 − 26 = 6 — nearer 32, so log 26 → 5"]][[step eq="16 < 26 < 32 → log 26 → 5"]]'),
        ...
        "recap": [
            ...
            ("And that is a ragged logarithm, placed: log 18 sits between 4 and 5, closest to 4.",
...
_ALGEBRA2_U7 = [
    {
        ...
        "why": [
            ("Why ride? Because a pattern with a steady step is a road you can ride. "
             "4, 7, 10, 13 — it starts at 4 and grows by 3. To find a far-off term, you could walk: add 3, add 3, add 3, and on and on. Or you could RIDE: count the steps and jump there in one move.",
        ...
        "recap": [
            ("So, here it is again. A pattern with a steady step is ridden, not walked: the start, plus the step taken one fewer time than the term number. Term 1 is already standing at the start. Posts and rails.",
             '[[bars data="term 1:4 | term 2:7 | term 3:10 | term 4:13" caption="terms are the posts; the ride is the rails"]]'),
            ("And that is the fencepost, in a new coat: 4 plus 9 times 3 is 31.",
             '[[step eq="4 + 9 × 3 = 31"]]'),
        ],
        "bank": [
            {"a": 2, "b": 3, "c": 4, "op": "anth"},
    ...
    },
    {
        ...
        "picture": [
            ("Here is the sum 1 up to 10 as a staircase. Ten rows, each one longer than the last, inside a rectangle 10 rows tall and 11 wide. The staircase is "
        ...
        "pairs": [
            {"worked": ("Here is one more, done for you. 1 up to 13: thirteen numbers do "
                        "not pair off evenly, so use the rectangle. 13 rows of 14 hold "
                        "two copies of the sum — 13 times 14 is 182, halved: 91.",
        ...
        "recap": [
            ("So, here it is again. To add the counting numbers from 1 up to any "
             "number, pair the ends — every pair is one more than the last number. "
             "Then times the last number by one more than itself, and halve. The rectangle holds two copies of the sum, and the staircase is half of it.",
             '[[rectangle w="11" h="10" half="1" caption="pair the ends, times, halve"]]'),
            ("And that is what Gauss saw at nine years old: 10 times 11, halved, is 55.",
             '[[step eq="10 × 11 ÷ 2 = 55"]]'),
        ],
        "bank": [
            {"a": 4, "b": 0, "op": "gaus"},
    ...
    },
    {
        ...
        "picture": [
            ("Here is the rule as a machine: times 2, then take away 1. Feed term 1 in "
             ...
             '[[machine input="5" rule="2x − 1" output="9" caption="term 1 in, term 2 out — then 9 goes back in"]][[machine input="9" rule="2x − 1" output="17" caption="term 2 in, term 3 out"]]'),
        ],
        "teach": [
            ...
            ("Two traps, both about not finishing. Stopping at term 2 answers a "
             ...
             '[[step eq="5 → 9 → 17 ✓"]][[step eq="9 ✗ stopped early"]][[step eq="20 ✗ doubling only: 5 → 10 → 20"]]'),
        ],
        "pairs": [
        ...
        "recap": [
            ("So, here it is again. A rule with no shortcut is walked: apply the whole of it to the term before, and feed each answer back in. Stop when you arrive at the term that was asked for. All of the rule, every term.",
             '[[machine input="5" rule="2x − 1" output="9" caption="apply the whole rule, every term"]]'),
            ("And that is a pattern walked, not ridden: 5, then 9, then 17.",
             '[[step eq="5 → 9 → 17"]]'),
        ],
        "bank": [
            {"a": 3, "b": 2, "op": "reca"},
...
_ALGEBRA2_U8 = [
    {
        ...
        "recap": [
            ("So, here it is again. The sine of an angle is the height of the arrow's tip on the unit circle. 1 straight up, negative 1 straight down, 0 when the arrow lies flat. Strip away the full turns first — they "
             "change nothing.",
    ...
    },
    {
        ...
        "why": [
            ("Why the across? Because the sine read the arrow's height, and its partner, the cosine, reads the ACROSS. That is how far right or left of the middle the tip sits. Same circle, same arrow, other coordinate: sine is "
        ...
        "recap": [
            ...
            ("And that is the sine's partner: right is 1, left is negative 1, straight up or down is 0.",
    ...
    },
    {
        ...
        "recap": [
            ("So, here it is again. A full turn is 360 degrees, and adding it keeps the arrow in the same place. It lands where it began, with the same sine and the same cosine. A half turn lands opposite, and 360 minus the angle is a mirror, not one more full turn.",
             '[[unitcircle angle="405" values="0" caption="same direction, same sine, same cosine"]]'),
            ("And that is why the wave repeats forever: 45 degrees plus 360 degrees is 405 degrees, the same arrow.",
             '[[step eq="45° + 360° = 405°"]]'),
        ],
        "bank": [
            {"a": 10, "b": 0, "op": "spin"},
    ...
    },
    {
        ...
        "recap": [
            ...
            ("And that is a wave, stretched and read: the crest is 20.",
             '[[step eq="crest = 20"]]'),
        ],
        "bank": [
            {"a": 2, "b": 0, "op": "ampl"},
...
_ALGEBRA2_U9 = [
    {
        ...
        "recap": [
            ("So, here it is again. When scores repeat, every score goes into the mean as many times as it happened. A score that happens three times pulls three times as hard. Never average the two numbers alone.",
    ...
    },
    {
        ...
        "why": [
            ("Why three slots? Geometry counted outfits from two choices: shirts times "
             ...
             "slot. The choices in each slot TIMES together when every choice in a slot can go with every combination made so far. 2 shirts, 5 pants, 3 hats — for each shirt, "
        ...
        "recap": [
            ("So, here it is again. Every category is a slot. When every choice fits every combination so far, the slots times together, however many there are: shirts times pants times hats. Never add the "
             "closets, and never stop a slot early.",
             '[[array rows="2" cols="5" caption="times every slot, skip none"]]'),
            ("And that is the counting principle, one slot longer: 2 times 5 times 3 is 30.",
             '[[step eq="2 × 5 × 3 = 30"]]'),
        ],
        "bank": [
            {"a": 2, "b": 2, "c": 2, "op": "cnt3"},
    ...
    },
    {
        ...
        "why": [
            ("Why expect? Because chance can be planned for. A game pays 5 tokens, and "
             "you win about 2 times out of every 6 plays. Play 6 times: you cannot "
        ...
        "recap": [
            ("So, here it is again. To know what to expect from a game like these, "
             "count the plays that pay, then times by the prize. Not every play pays, "
             "and one win is not the whole story — the expected amount is a long-run "
             "average, never a promise.",
             '[[pie parts="6" shaded="2" caption="count the paying plays, then times"]]'),
            ("And that is chance, planned for: 2 times 5 is 10 expected.",
    ...
    },
    {
        ...
        "why": [
            ("Why does a sample speak? Because the last idea of Algebra Two is the "
             ...
             "says a fair sample — one that looks like the school — SPEAKS for the "
             "school, about.",
             '[[goal text="The sample speaks"]]'),
        ],
        "picture": [
            ("Here is the sample beside the school as bars: 20 asked, 5 said yes, and "
             "a school of 60 standing three samples wide. If every slice of 20 behaves "
