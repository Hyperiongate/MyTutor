...
#               solid (HIGH, and "25 times 12 is 300 pi" was arithmetic that read as
#               300π = 25 × 12 -- HIGH); the chain rule is not its front number (HIGH);
#               three laws get their condition (the limit laws in the advance line, the
#               derivative of THIS line, the area from the antiderivative at the two ends);
#               the gap is not the height you were ASKED for (a gap CAN equal a height when
#               the lower curve is zero); rate times time needs a steady rate. The rest:
#               six more rules scoped, "initial condition" and "growth constant" taught
#               where they are used, six boards drawing what the words say.
...
#               words did not read is read now -- 19 lessons ("And that is the chain rule:
#               6 times 5 is 30"); (2) every sentence of 27 words or more in a why, picture,
#               teach, worked or recap beat is split -- 45 sentences. No beat added, no
#               number changed, no board changed.
#   2026-09-17  BUILD wr -- THE SECOND CALCULUS SWEEP, WHOLE (88 findings, 68 authored, 4 of
#               36 clean; 72 edits here). The first whole read of the 17 lessons wo never
#               reached (U5's where-the-bend-changes, U6-U9) and a re-read of the rest.
#               LAWS WITH THEIR CONDITION: the limit passes through "for sums and
#               products, when each function has a limit of its own"; a line "written y
#               equals a number times x, plus a number" (HIGH); a derivative FUNCTION "is not
#               just one number"; "for this fence problem" the best sits inside; "every fixed
#               fence used for a rectangle this way"; the last lesson's square-wins fact is
#               named before the 5-by-15 rival; the slope-zero side of 10 IS the top --
#               "the area curve is a dome" (HIGH); "for these cubics, set the second
#               derivative to zero, then check the bend changes sides" (HIGH, and the
#               advance line); "the critical-point pattern you met" halved; "a zero slope
#               does not tell you where; the change in the slope does"; an antiderivative
#               is "a function it COULD have come from" (HIGH); the reverse power rule is
#               "for the whole-number powers in this lesson" and names x to the negative 1
#               as the one it cannot serve (HIGH x2); "6 x to the fourth is ONE function it
#               came from -- plus any constant"; going backwards "gives a whole family of
#               antiderivatives" (HIGH); "the GENERAL antiderivative is written with plus
#               C" (HIGH); 5 apart everywhere "because these curves share one derivative";
#               "the one curve the initial condition picks" with y(0) = 7 on the board
#               (HIGH); the definite integral "for a speed graph that stays above the
#               axis"; "for this ramp, starting at zero with speed equal to t" (recap and
#               advance line); the Fundamental Theorem's advance line is "work the
#               antiderivative out at both ends, then end take away start" (HIGH -- it said
#               "the smaller from the bigger"); "under a graph that stays above the axis";
#               "when one curve stays on top the whole way" (teach and recap); "the halfway
#               speed is the HEIGHT of a rectangle with the same area" (HIGH -- the line
#               does not cut the trapezium into one); "because we are integrating a flow
#               rate, the integral measures the CHANGE" (HIGH); "every slice STRAIGHT
#               ACROSS that cylinder" and "for these rectangles spun about the line beneath
#               them" (HIGH x2); "MOST equations you have met describe an amount" (HIGH);
#               the rate equation says only how steeply the line drops -- "it is the
#               starting 60 that lets the line show how much is in there" (HIGH); the
#               net-rate tank "starting empty" (HIGH); "in this lesson the differential
#               equation tells you how fast a population changes"; "40 is not the
#               EQUILIBRIUM population" (HIGH); "a POPULATION whose rate is 45 take away 5 P
#               stops changing at 9" (HIGH); the derivative EQUATION used backwards; "for a
#               distance-after-time formula"; "for a stone falling straight down"; 5 is
#               "the distance's original front number, before any differentiating"; "in
#               this model that constant is gravity"; the plus number "is not the height at
#               the border" (HIGH -- at x = 0 it is one); the mend-the-curve distractor no
#               longer states a false law; "the quantity 4 x plus 7, squared" in the reason
#               question (HIGH -- without it the slope is 4). WORDS-BOARD: every closing
#               beat whose board wrote an equation reads it (ten lessons); the x² -> 2x
#               comparison, the 2x = 6 step, the 8-added trap and y(0) = 7 are drawn or
#               said; the worked 9x⁴ says the power drops to 3; the ramp's closing beat
#               keeps its picture; the accumulation recap draws the FLOW graph it talks
#               about; the flatten picture says the flat top comes next; the volume's
#               closing board was the false equation "3² × 4 = 36 · 36π" (HIGH) -- two
#               lines; P is named before "rate = 4P". UNCLEAR: nine long sentences split.
#               Generator items: lessonscripts.py (wr).
...
_CALCULUS_U1 = [
    {
        ...
        "teach": [
            ("That is the method: for sums and products, when each function has a limit "
             "of its own, the limit passes straight through the arithmetic. If f is "
             "heading for 6 and g for 5, then f plus g heads for 11 and f times g heads "
             "for 30. The sum of the limits; the product of the limits. Here is the "
        ...
        "recap": [
            ("So, here it is again. Limits pass straight through the arithmetic when each function has a limit of its own. For a product, f times g heads for the product of the two limits. Adding answers a different question, and the bigger "
            ...
            ("And that is the limit law every later rule rests on: 6 times 5 is 30.",
             '[[step eq="6 × 5 = 30"]]'),
        ],
        "bank": [
            {"a": 3, "b": 2, "op": "llaw"},
    ...
    },
    {
        ...
        "why": [
            ("Some limits are asked at the far end of the number line, where x runs off "
             ...
             "settled to a number. Calculus asks this kind in general. For fractions carrying the same top power the method is always the same: far out, only the leaders matter.",
        ...
        "teach": [
            ...
            ("For the settling number, nothing else about the two pieces changes it. Taking one number from the other, or timesing them, describes no part of what the fraction does. "
        ...
        "recap": [
            ...
            ("And that is a limit at infinity: 8 over 2 is 4.",
             '[[step eq="8 ÷ 2 = 4"]]'),
        ],
        "bank": [
            {"a": 6, "b": 3, "op": "linf"},
    ...
    },
    {
        ...
        "teach": [
            ("That is the method: take the lower side from the higher one. From the left the curve "
             "heads for 4; from the right it heads for 10. It leaps 6 in no distance at all, so the break has a size: 6. Mathematicians call this a jump discontinuity, for the obvious reason.",
        ...
        "recap": [
            ...
            ("And that is how big the break is: 10 take away 4, a jump of 6.",
             '[[step eq="10 − 4 = 6"]]'),
        ],
        "bank": [
            {"a": 3, "b": 5, "op": "jump"},
    ...
    },
    {
        ...
        "explain": {
            ...
            "choices": ("because the flat piece has to meet the slope where it arrives | "
                        "because the flat piece keeps the value it was given | "
        ...
        "recap": [
            ("So, here it is again. To mend a break, walk the sloping piece to the border. "
             "Set the flat piece to meet the sloping piece there, so the limit coming in "
             "equals the value waiting. The broken value is what you were given, and the "
             "plus number in the sloping piece is not the height at the border.",
             '[[graph func="x+2 for x<5; 7 for x>=5" points="(5,7)" range="0..9" yrange="0..11" caption="the two ends meet"]]'),
            ("And that is continuity as a repair job: 5 plus 2 is 7.",
             '[[step eq="5 + 2 = 7"]]'),
        ],
        "bank": [
            {"a": 2, "b": 4, "c": 3, "op": "cfix"},
...
_CALCULUS_U2 = [
    {
        ...
        "recap": [
            ...
            ("And that is the derivative: at 4, the slope is 8.",
    ...
    },
    {
        ...
        "why": [
            ("Shrinking a window every time would be unbearable, so Calculus finds the "
             "pattern once and keeps it. It is called the power rule, and it turns a whole limit into two moves. The exponent comes down in front, and the power drops by one.",
             '[[goal text="The power comes down front"]]'),
        ],
        "picture": [
            ("Here is the rule as a machine. For 6 x to the power 3, the exponent is 3. "
             "The 3 goes in and meets the 6 already standing there. Out comes the front "
             "number of the derivative. Watch what the exponent does: it comes down and "
             "times the front.",
        ...
        "teach": [
            ("That is the method, and it agrees with what you know. For x squared the 2 "
             ...
             '[[step eq="x² → 2x"]][[machine input="3" rule="× 6" output="18" caption="3 times 6 is 18, and the power drops to 2"]][[write text="18x^2"]][[step eq="3 × 6 = 18"]]'),
        ...
        "pairs": [
            {"worked": ("Here is one more, done for you. For 9 x to the power 4: the 4 comes down onto the 9, giving 36. The power drops to 3, so the derivative is 36 x to the power 3.",
        ...
        "recap": [
            ...
            ("And that is the front-number part of the power rule: 3 times 6 is 18.",
             '[[step eq="3 × 6 = 18"]]'),
        ],
    ...
    },
    {
        ...
        "advance_line": "Three in a row, and you can say why — you've got it! Written y equals a number times x plus a number, a line's slope is the number in front of x.",
        "why": [
            ("Not every derivative changes as you move. A line written y equals a number "
             "times x, plus a number, has the same steepness at every point on it. So "
             "its derivative is a constant — one number, true everywhere along the line. "
             "Lines are the easiest curves in Calculus, and the best place to check the "
             "rules agree.",
        ...
        "teach": [
            ("That is the method: read the number sitting in front of x. For y equals 3 "
             "x plus 5, the slope is 3 wherever you stand. In Algebra One, that was the "
             "climb per step across. In Calculus, the same number is the derivative. The "
             "power rule agrees: x is x to the 1, and the 1 comes down onto the 3.",
            ...
            ("The 5 does nothing to the steepness; it only lifts the whole line up the page. A plain number on its own has a derivative of zero. Adding the "
             "slope to the lift — the crossed-out 8 — is not the derivative either. Read "
             "the number in front of x, and ignore the one standing alone.",
        ...
        "recap": [
            ("So, here it is again. A line written y equals a number times x, plus a number, has one slope: the number in front of x. That is its derivative everywhere. The number standing alone only lifts "
            ...
            ("And that is the derivative of this line: its slope, 3.",
             '[[step eq="slope = 3"]]'),
        ],
    ...
    },
    {
        ...
        "why": [
            ("A derivative is not just one number. It is a whole new function: a machine that hands you the slope number at whatever x you feed it. So "
             "there are two jobs, in order: get the derivative first, then feed it the x "
        ...
        "recap": [
            ...
            ("And that is evaluating a derivative: 10 times 6 is 60.",
             '[[step eq="10 × 6 = 60"]]'),
        ],
...
_CALCULUS_U3 = [
    {
        ...
        "recap": [
            ...
            ("And that is the product rule, checked: 2 times 5, plus 4, is 14.",
             '[[step eq="2(5) + 4 = 14"]]'),
        ],
    ...
    },
    {
        ...
        "recap": [
            ("So, here it is again. The chain rule: differentiate the outside, then times by the derivative of the inside. The power comes down, and the inside's derivative multiplies it. Forgetting the inside is the commonest mistake in Calculus, and adding the two is no rule at all.",
             '[[machine input="6" rule="× 5" output="30" caption="the power AND the inside"]]'),
            ("And that is the chain rule's front number: 6 times 5 is 30.",
             '[[step eq="6 × 5 = 30"]]'),
        ],
    ...
    },
    {
        ...
        "show_work_on_correct": True,
        "explain": {
            "spoken": ("One more thing — not the answer, the reason. For the quantity 4 x "
                       "plus 7, squared, the slope at zero is 56, not 14. Tap the reason "
                       "why."),
        ...
        "recap": [
            ...
            ("And that is the chain rule, fed a number: 2 times 7 times 4 is 56.",
             '[[step eq="2 × 7 × 4 = 56"]]'),
        ],
    ...
    },
    {
        ...
        "recap": [
            ...
            ("And that is what a constant denominator does: the 4 stays put, and 24 over 4 is 6.",
             '[[step eq="24 ÷ 4 = 6"]]'),
        ],
...
_CALCULUS_U4 = [
    {
        ...
        "why": [
            ("Unit Four puts derivatives to work. A derivative hands you a rate formula, "
             "and a formula set equal to a target is an equation you can solve. If a falling stone's speed is 8 t metres a second, asking WHEN it falls at 40 is a question about t. What t turns 8 t into 40? That is the derivative's equation used "
             "backwards: from a speed value to the time that gives it.",
             '[[goal text="When is it going that fast?"]]'),
        ],
        "picture": [
            ("Here is the speed as a line: 8 metres a second faster with every second "
             "that passes. Somewhere along that line the speed reaches 40. Find the height 40 on the side, run your finger across to the line, and drop down to the time underneath. That moment is what the question wants.",
        ...
        "teach": [
            ...
            ("Watch which number you divide by. That speed came from a distance of 4 t squared, and the speed's front number is DOUBLE the distance's. Dividing by the distance's 4 gives the wrong time. And handing back 40 answers with "
        ...
        "recap": [
            ("So, here it is again. For a distance-after-time formula, the derivative gives "
             "the speed formula; set that speed equal to the number you were given and "
             "solve for the time. For these t-squared distance formulas the speed is a front number times t. The speed is not the time, and the number you divide by is the speed's front number, not the distance's.",
             '[[graph lines="y=8x; y=40" names="speed = 8t; speed = 40" points="(5,40)" range="0..7" yrange="0..56" caption="set the speed equal and solve"]]'),
            ("And that is how a derivative answers WHEN: 8 t equals 40, so t is 5.",
    ...
    },
    {
        ...
        "why": [
            ("Here is what derivatives are really for. A square's side is growing "
             "steadily. Its area is growing too — but not steadily at all, because a "
             "bigger square gains more area from the same extra centimetre. One rate "
        ...
        "teach": [
            ("That is the method. Area is side squared, so the chain rule applies: the area's rate is 2, times the side, times the side's own rate. With a side "
            ...
            ("And when that same square reaches a side of 50, the area gains 2 times 50 times 3, which is 300 a second. The same steady growth, a bigger square. So do not hand back 3, the side's rate, or the area itself, 225 at a side of 15. The question asks how fast the area CHANGES.",
        ...
        "recap": [
            ("So, here it is again. For this square, as its side grows the area grows too, and the chain rule says how fast. The area's rate is 2 times the side times the side's rate. Never hand back the side's rate alone, and never "
             "the area itself.",
             '[[graph func="x^2" names="area = side²" lines="y=30x-225" points="(15,225)" range="0..18" yrange="0..324" caption="one rate drives another"]]'),
            ("And that is a related rate: 2, times the side, times the side's rate.",
    ...
    },
    {
        ...
        "teach": [
            ("That is the method: differentiate, set the slope to zero, solve. The slope "
             ...
             '[[graph func="x^2-6*x" names="y = x² − 6x; the flat tangent" lines="y=-9" points="(3,-9)" range="0..6" yrange="-11..2" caption="flat at x = 3 — the bottom of the valley"]][[step eq="2x − 6 = 0"]][[step eq="2x = 6"]][[step eq="x = 3"]]'),
        ...
        "pairs": [
            ...
            {"worked": ("One more together. A slope of 2 x take away 34: set it to zero, so 2 x equals 34 and x is 17. The curve levels off there.",
        ...
        "recap": [
            ("So, here it is again. A smooth curve turns only where it is flat, and flat "
             ...
             "The number in the slope is what you set 2 x equal to. The answer is half of it, never the number itself and never double.",
            ...
            ("And that is a critical point: 2 x take away 6 equals zero, so x is 3.",
    ...
    },
    {
        ...
        "why": [
            ("Differentiating does not have to stop after one go. For a stone falling "
             "straight down, distance differentiates into speed; differentiate the SPEED "
             "and you get acceleration — how fast the speed itself is changing. "
             "Differentiate twice, and the falling stone hands you gravity.",
        ...
        "picture": [
            ("Here is the speed of a stone that has fallen 5 t squared metres: its speed "
             ...
             '[[step eq="fallen 5t² metres"]][[graph lines="y=10x" names="speed = 10t" range="0..5" yrange="0..50" caption="the speed line — it climbs the same amount every second"]]'),
        ],
        "teach": [
            ("That is the method: differentiate the speed. 10 t is a line, and a line's "
             ...
             "One second after that, 10 higher again.",
            ...
            ("In this model that constant is gravity, and finding it took two "
             "differentiations. 5 is the distance's original front number, before any "
             "differentiating; and 20 doubles the 10 once too many times, which no "
             "derivative does here.",
             '[[step eq="10 ✓"]][[step eq="5 ✗ the original front number · 20 ✗ doubled again"]]'),
        ],
        "pairs": [
...
_CALCULUS_U5 = [
    {
        ...
        "teach": [
            ...
            ("Every fixed fence used for a rectangle this way wants a square; long thin "
             ...
             "the fence four ways. Half of it is two sides at once, and the whole 40 is "
             "no side at all.",
    ...
    },
    {
        ...
        "teach": [
            ("That is the method: the area is the side squared. 10 times 10 is 100 "
             "square metres — ten rows of ten squares. The last lesson showed the square wins for a fixed fence. Here is one rival: 5 by 15 uses the identical fence and encloses only 75.",
        ...
        "recap": [
            ("So, here it is again. For a fixed amount of fence the best rectangle is a square, and the ground it encloses is the side squared. Rows of squares, each row as long as the side. The side is not the ground, and the fence is not the ground either.",
             '[[rectangle w="10" h="10" show="area" caption="side squared is the ground"]]'),
            ("And that is how much the best rectangle wins: 10 times 10, 100 square "
             "metres.",
             '[[step eq="10 × 10 = 100"]]'),
        ],
    ...
    },
    {
        ...
        "picture": [
            ("Here is y equals x cubed take away 6 x squared. Watch the bend, not the "
             ...
             "scoops up like a cup. Somewhere between, the bend changes sides — a zero "
             "slope does not tell you where; the change in the slope does.",
        ...
        "teach": [
            ...
            ("Notice the arithmetic differs from the critical-point pattern you met: "
             ...
             "wrong x, and the number in the equation is not the answer either.",
             '[[step eq="2 ✓"]][[step eq="3 ✗ the halving habit · 6 ✗ the equation\'s number"]]'),
        ],
        "pairs": [
            ...
            {"worked": ("One more together. 6 x take away 168: 6 x equals 168, so x is 168 divided by 6, which is 28. That is where the dome becomes a cup.",
        ...
        "recap": [
            ...
            ("And that is an inflection point: 6 x take away 12 is zero, so x is 2.",
...
_CALCULUS_U6 = [
    {
        ...
        "why": [
            ("Every unit so far has asked what the derivative of a function is. Unit Six "
             "asks it backwards. Given a derivative, what function could it have come "
             "from? Such a function is called an antiderivative. There is more than one, a constant apart, as the third lesson shows. Finding one means running the power rule in reverse.",
        ...
        "teach": [
            ("That is the method: HALVE the front number, and put the power back up to "
             ...
             '[[machine input="6" rule="÷ 2" output="3" caption="6 halved is 3 — the front number of the function it came from"]][[write text="3x²"]][[step eq="3x² → 6x"]][[step eq="6x → 3x²"]]'),
            ("So halve, do not double. Doubling runs the forward rule the wrong way and lands twice as high. Copying the number straight over forgets that differentiating doubled it in the first place.",
    ...
    },
    {
        ...
        "why": [
            ("Halving worked because the power was 2. For the whole-number powers in this lesson the reverse move is the same shape. Raise the exponent by one, then divide the front number by that new exponent. Two steps, in that "
             "order, every time here — the one power it cannot serve, x to the negative "
             "1, waits for a later course.",
             '[[goal text="Raise, then divide"]]'),
        ],
        "picture": [
            ("Here is the reverse move as a machine. A derivative of 24 x cubed: the exponent 3 climbs to 4. The front number 24 goes into the machine to be divided by that new 4. What comes out is the front number of the "
        ...
        "teach": [
            ("That is the method: raise, then divide. The power 3 climbs to 4, and 24 "
             "divided by 4 is 6. So 6 x to the fourth is one function it came from — "
             "plus any constant, as the next lesson but one explains. Check it forwards: "
        ...
        "pairs": [
            ...
            {"worked": ("One more together. 108 x to the eighth: the power climbs to 9, and 108 over 9 is 12. So 12 x to the ninth is the function it came from.",
        ...
        "recap": [
            ("So, here it is again. To reverse the power rule on the powers in this "
             "lesson: raise the exponent by one, then divide the front number by that "
             "new exponent. Never skip the dividing, and never hand back the exponent as "
             "if it were the front number.",
             '[[machine input="24" rule="÷ 4" output="6" caption="raise, then divide"]]'),
            ("And that is the reverse power rule: 24 x cubed came from 6 x to the fourth.",
             '[[step eq="24x³ → 6x⁴"]]'),
        ],
    ...
    },
    {
        ...
        "why": [
            ("Here is the catch in running the rule backwards. A flat number "
             ...
             "derivative. Going backwards cannot tell them apart — it gives a whole "
             "family of antiderivatives, a constant apart.",
        ...
        "teach": [
            ("That is the method: same derivative means a fixed gap, so add the gap on. "
             ...
             "reads 17 there — 12 plus 5. Because these curves share one derivative, "
             "being 5 apart at one x means 5 apart at every x.",
            ...
            ("Add the gap on. Taking it away drops you below the curve you were given. The gap on its own is not the height you were asked for; it is the plus C the general antiderivative carries.",
        ...
        "recap": [
            ...
            ("And that is why the general antiderivative is written with plus C: 12 plus "
             "5 is 17.",
             '[[step eq="12 + 5 = 17"]]'),
        ],
    ...
    },
    {
        ...
        "recap": [
            ...
            ("That starting fact, y of zero equals 7, is called the initial condition. And that is the one curve it picks: y equals x squared plus 7.",
             '[[step eq="y(0) = 7"]][[step eq="y = x² + 7"]]'),
        ],
...
_CALCULUS_U7 = [
    {
        ...
        "advance_line": "Three in a row, and you can say why — you've got it! For a steady speed, height times width is the area under the graph.",
        "why": [
            ("Unit Six ran the derivative backwards. Unit Seven does something that "
             ...
             "underneath a graph. That area is what a definite integral measures, for speeds like this unit's that never dip below zero. The simplest one of all is a rectangle.",
        ...
        "teach": [
            ("That is the method: height times width. The rectangle is 8 tall and 5 wide, "
             ...
             "MEANS the distance. For a speed graph above the axis, that is what the "
             "integral is: the area underneath, read as whatever the picture measures.",
            ...
            ("So multiply the height by the width. Adding them puts metres a second and seconds together as though they measured the same thing. The width alone is just the time you were already told.",
    ...
    },
    {
        ...
        "advance_line": "Three in a row, and you can say why — you've got it! For this speed-equals-t ramp: square the time, then halve it.",
        "why": [
        ...
        "recap": [
            ("So, here it is again. Under this ramp, starting at zero with speed equal to t, the area is a triangle. A triangle is half the rectangle around it: square the time, then halve. Forget the half and you have claimed the "
             "whole rectangle; the time alone is not a distance.",
             '[[graph lines="y=x" names="speed = t" shade="0..6" label="18" range="0..8" yrange="0..8" caption="when the graph is a ramp"]]'),
            ("And that calculation gives the area under the ramp: 6 times 6, halved, 18.",
             '[[graph lines="y=x" names="speed = t" shade="0..6" label="18" range="0..8" yrange="0..8" caption="the triangle under the ramp"]][[step eq="6 × 6 ÷ 2 = 18"]]'),
        ],
    ...
    },
    {
        ...
        "advance_line": "Three in a row, and you can say why — you've got it! Work the antiderivative out at both ends, then end take away start.",
        "why": [
            ("Rectangles and triangles are easy shapes. Most graphs are neither. So here is the theorem that handles all of them, and it is the biggest idea in the subject. For the graphs in this course, the area comes from the antiderivative at the two ends.",
        ...
        "teach": [
            ("That is the method. Run the rule backwards as in Unit Six: 2 x comes from x squared. Then work x squared out at both ends and take one from the other. From 3 to 5: 25 take away 9 is 16, and the shaded strip holds "
             "exactly 16.",
        ...
        "explain": {
            ...
            "choices": ("because the end x-value squared take away the start x-value squared | "
                        "because the gap between the ends is squared | "
                        "because the area is the width of the strip"),
            "answer": "because the end x-value squared take away the start x-value squared",
    ...
    },
    {
        ...
        "picture": [
            ("Here is a curve with a hump and a dip, and the shaded area under it from 0 "
             ...
             "flat — the next board draws that flat top. The area does not change; only "
             "the shape does.",
        ...
        "teach": [
            ("That is the method: area divided by width. An area of 60 spread across a width of 5 stands 12 high. So 12 is the curve's average height: the average value of the function. The flat line sits exactly where the hump "
...
_CALCULUS_U8 = [
    {
        ...
        "teach": [
            ("That is the method: top area take away bottom area. The 18 under the bottom curve is already counted inside the 50, so take it away. 50 take away 18 is 32, and the strip between them holds 32.",
            ...
            ("When one curve stays on top the whole way across the stretch, it is top "
             "take away bottom, in that order. Adding the two counts the lower region "
             "twice over, and answering with the top area alone hands back the whole "
             "slab instead of the gap.",
        ...
        "recap": [
            ("So, here it is again. On the same stretch, when one curve stays above the other, the area between them is the top area take away the bottom area. The bottom area is already inside the top. Never add them, and "
             "never hand back the top area alone.",
            ...
            ("And that is the area between: 50 take away 18 is 32.",
             '[[step eq="50 − 18 = 32"]]'),
        ],
    ...
    },
    {
        ...
        "why": [
            ("A rectangle came from a steady speed and a triangle from a speed starting "
             "at nothing. Now a train that is already moving speeds up further. The graph is a ramp that starts partway up the page, and the shape under it is a trapezium.",
        ...
        "teach": [
            ("That is the method: average the two speeds, then hold it for the time. The speed climbs steadily, so the average speed is exactly halfway between 4 and 10: that is 7. And 7 metres a second for 5 seconds is 35 metres. The "
             "halfway speed is the height of a rectangle with the same area as the "
             "trapezium.",
    ...
    },
    {
        ...
        "why": [
            ("Areas have measured distances so far. Here is the same idea measuring "
             "something you can pour. A tank holds 20 litres at the start, and water runs in at 6 litres a minute for 5 minutes. The integral only counts what ARRIVES.",
        ...
        "teach": [
            ("That is the method: work out the change, then add it to what was there. "
             ...
             '[[graph lines="y=6x+20" names="litres in the tank" points="(0,20),(5,50)" range="0..5" yrange="0..60" caption="from 20 litres at the start up to 50 after 5 minutes"]][[step eq="6 × 5 = 30"]][[step eq="20 + 30 = 50"]]'),
            ("This is what accumulation means: here, because we are integrating a flow "
             "rate, the integral measures the CHANGE, not the amount already there. "
             "Answering 30 forgets the water that was already there, and adding all "
             "three numbers loosely gives 31, which measures nothing at all.",
        ...
        "recap": [
            ("So, here it is again. The area under a flow graph is what arrives — the "
             "change — and integrating a rate measures the change, not the amount. Add "
             "it onto what was already there. Never forget the start, and never add the "
             "three numbers loosely.",
             '[[graph lines="y=6" names="6 litres a minute running in" shade="0..5" label="30" range="0..7" yrange="0..8" caption="what arrives — 30 litres, on top of the 20 already there"]]'),
            ("And that is accumulation: 20 plus 30 is 50.",
             '[[step eq="20 + 30 = 50"]]'),
        ],
    ...
    },
    {
        ...
        "picture": [
            ("Here is a rectangle 3 tall and 4 long. Spin it about the line beneath it and it sweeps out a cylinder. The height 3 becomes the radius, and the length 4 stays the length. Every slice straight across that cylinder is a "
             "circle of radius 3.",
        ...
        "pairs": [
            {"worked": ("Here is one more, done for you. A rectangle 6 tall and 8 long: 6 squared is 36, so each slice is 36 pi, and 8 of those slices stack to 288 pi.",
                        '[[solid kind="cylinder" r="6" h="8" caption="a circle of area 36π, stacked 8 long — 288π"]][[step eq="6² × 8 = 288"]]'),
             "ask": {"a": 5, "b": 8, "op": "revo"}},
            {"worked": ("One more together. 5 tall and 12 long: 25 times 12 is 300, so the volume is 300 pi — the circle of area 25 pi, stacked twelve long.",
        ...
        "recap": [
            ("So, here it is again. Spin a flat shape about a line and it sweeps out a "
             "solid. For these rectangles spun about the line beneath them, every slice straight across is a circle. So square the radius and stack the circles along the length. Leave the squaring out and you still have a flat area; "
             "double where you should square and the solid comes out thin.",
             '[[solid kind="cylinder" r="3" h="4" caption="spin it into a solid"]]'),
            ("And that is a volume of revolution: 3 squared times 4 is 36, so 36 pi.",
             '[[step eq="3² × 4 = 36"]][[step eq="volume = 36π"]]'),
        ],
...
_CALCULUS_U9 = [
    {
        ...
        "why": [
            ("Most equations you have met describe an AMOUNT. A differential equation describes a rate instead — how fast something is changing. Integration, together with the starting amount, turns it back into an amount. Unit Nine reads three "
             "of them.",
        ...
        "picture": [
            ("Here is a tank that starts at 60 litres, and the equation says it loses 4 "
             ...
             "falls 4 for every minute across. The rate equation alone says only how "
             "steeply the line drops; it is the starting 60 that lets the line show how "
             "much is in there.",
             '[[graph lines="y=-4x+60" names="litres in the tank" points="(0,60)" range="0..9" yrange="0..70" caption="the rate equation says the line falls 4 every minute — the start at 60 says where it falls from"]]'),
        ],
        "teach": [
        ...
        "recap": [
            ...
            ("And that is how a rate equation is read against the clock: 60 take away 4 "
             "times 7 is 32.",
             '[[step eq="dV/dt = −4, start 60"]][[step eq="60 − 4 × 7 = 32"]]'),
        ],
    ...
    },
    {
        ...
        "teach": [
            ("That is the method: settle the fight before you touch the clock. Nine in "
             ...
             "number is the net rate. Starting empty, over 7 minutes it gains 35 litres, "
             "and the line of what is in the tank climbs 5 every minute.",
        ...
        "explain": {
            ...
            "board": '[[bars data="in:9 | out:4" caption="why 35, and not 91?"]][[step eq="after 7 min · 35, not 91"]]',
        },
        "recap": [
            ("So, here it is again. When two rates pull against each other, find the net rate first: in take away out. Only then let the clock work on it. "
            ...
            ("And that is a net rate: 9 take away 4, times 7, is 35.",
             '[[step eq="(9 − 4) × 7 = 35"]]'),
        ],
    ...
    },
    {
        ...
        "why": [
            ("Both tanks so far changed at a fixed rate. Now the most important differential equation there is: the one where the rate depends on the amount itself. Growth that is proportional to what is already there. A "
             "bacterial colony is the classic.",
             '[[goal text="When the rate depends on the amount"]]'),
        ],
        "picture": [
            ("Here is the rate drawn against the amount. Call the colony's size P: for "
             "every bacterium in the dish, the colony gains 4 more a minute, so the rate "
             "is 4 P. The line climbs — the bigger the colony, the faster it grows. Read "
             "it at any size and it tells you how fast the colony is growing right then.",
        ...
        "teach": [
            ("That is the method: multiply the amount by the growth constant, the 4 that every single bacterium adds each minute. With 12 "
            ...
            ("And that rate will not hold, because the growing feeds the growing. "
             "Adding the two numbers instead of timesing them gives 16. Answering 4 pretends a colony of a thousand grows no faster than a colony of ten.",
        ...
        "recap": [
            ...
            ("And that is proportional growth: 12 times 4 is 48.",
             '[[step eq="12 × 4 = 48"]]'),
        ],
    ...
    },
    {
        ...
        "why": [
            ("The last question of the whole course, and it is a quiet one. In this "
             "lesson the differential equation tells you how fast a population changes — "
             "so ask it where the change stops. That place is called equilibrium.",
        ...
        "teach": [
            ...
            ("It is a stable place, too. Above 9 the rate turns negative and pulls back "
             ...
             "not the equilibrium population, and 45 is the number in the equation, not "
             "the answer to it.",
        ...
        "show_work_on_correct": True,
        "explain": {
            "spoken": ("One more thing — not the answer, the reason. A population whose rate "
                       "is 45 take away 5 P stops changing at 9, not 40. Tap the reason why."),
        ...
        "recap": [
            ...
            ("And that is the end of the course: 5 P equals 45, so P is 9.",
