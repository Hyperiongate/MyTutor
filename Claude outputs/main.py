...
#               line. No change in this file beyond the stamp; prewarm 1 line. PART 3oa.
...
#               never settled). No change in this file beyond the stamp; nothing to prewarm.
#   2026-09-24  APP_BUILD -> "2026-09-24yb-the-words-grow-back-on-a-shrunk-figure". The over-tall
#               beat (the gate build xy's instrument named): a figure the board shrinks to fit
#               its turn is drawn again for the width it gets, labels re-fitted
#               (math-figures.js room, board.js figRedraw); screencheck S10 measures it. No
#               change in this file beyond the stamp; nothing to prewarm. PART 3nw.
#   2026-09-24  APP_BUILD -> "2026-09-24ya-the-third-geometry-sweep". The third Geometry reading
#               (11 findings on 36, 27 clean; 36 and 16 at xb): lessons/geometry.py (12 edits +
#               4 boards), static/math-figures.js ([[graph segments=]]). No change in this file
#               beyond the stamp; ~12 lines to prewarm. PART 3nv.
...
#               lessonscripts.py (farv, bias, ptre). No change in this file beyond the
#               stamp; ~60 lines to prewarm. PART 3nr.
...
#               PART 3nc, and 3nb's ratchet is now a pin at zero over all ten courses.
#               Course lines 39,970 -> 39,920, closure 40,224 -> 40,174, speechmap 941 of
#               40,226, drift 635. Nothing in this file changed but the stamp.
#   2026-09-21  APP_BUILD -> "2026-09-21xg-the-second-probstat-sweep". The second Prob/Stat
#               reading (60 findings on all 36, 10 clean; 92 and 1 at wo): 24 generator
#               praises became credit lines in lessonscripts.py, 38 authored edits in
#               lessons/probstat.py, PART 3nb. Course lines 40,005 -> 39,970, closure
#               40,259 -> 40,224, speechmap 1,001 of 40,276. Nothing in this file changed
#               but the stamp.
...
#   2026-09-19  APP_BUILD -> "2026-09-19xb-the-second-algebra1-sweep". The second Algebra I
#               reading (50 findings on all 36, 10 clean; 72 at wi): two generator ops in
#               lessonscripts.py, 42 authored edits in lessons/algebra1.py, PART 3mw.
#               Nothing in this file changed but the stamp.
...
#               five praise lines across three courses are credit lines (the walk-back
#               keeps the steps), eight authored edits in lessons/entry.py, PART 3mt.
#               Nothing in this file changed but the stamp.
#   2026-09-18  APP_BUILD -> "2026-09-18wx-the-second-algebra2-sweep". The second Algebra II
#               reading (43 findings on 35 of 36, 14 clean; 73 at wk): five generator ops in
#               lessonscripts.py, 39 authored edits in lessons/algebra2.py, PART 3ms. Nothing
#               in this file changed but the stamp.
...
#               in lessonscripts.py, 27 authored edits in lessons/precalc.py, PART 3mp.
#               Speechmap 2,244, drift 1,938. Nothing in this file changed but the stamp.
...
#               lessons/diffeq.py, the cost estimate read off the bill in coursesweep.py
#               (0.15 a lesson), PART 3mn. No count moved. Nothing in this file changed but
#               the stamp.
#   2026-09-17  APP_BUILD -> "2026-09-17wr-the-second-calculus-sweep". The second Calculus
#               sweep, whole (88 findings, all 36 read, 4 clean): thirteen generator ops in
#               lessonscripts.py, 72 authored edits in lessons/calculus.py, the admin card's
#               report list newest-first in coursesweep.py, PART 3mm. Every lesson in every
#               course has now been read once.
...
#   2026-09-16  APP_BUILD -> "2026-09-16wo-the-first-calculus-sweep-half". The first Calculus
#               sweep read 19 of 36 lessons (OpenAI credits ran out): six generator items in
#               lessonscripts.py, 62 authored edits in lessons/calculus.py (the dot split in
#               all 36), PART 3mj. The other 17 lessons wait for a rerun.
...
#   2026-09-16  APP_BUILD -> "2026-09-16wl-the-first-geometry-sweep".
#               BUILD wl -- no change in this file beyond the stamp. The first Geometry
#               sweep (63 findings): lessonscripts.py the generator items, lessons/geometry.py
#               the authored pile.
#   2026-09-16  APP_BUILD -> "2026-09-16wk-the-first-algebra1-sweep".
#               BUILD wk -- no change in this file beyond the stamp. The first Algebra I
#               sweep (72 findings): lessonscripts.py the generator items and the dot,
#               lessons/algebra1.py the authored pile, coursesweep.py two charter lines.
...
#               lessons/prealgebra.py the authored pile.
...
#               takes the other two (a why beat is a story, unpictured by design).
#   2026-09-15  APP_BUILD -> "2026-09-15wf-the-problem-space-on-the-page".
#               BUILD wf -- no change in this file beyond the stamp. coursesweep.py puts
#               the lesson's PROBLEM SPACE on the transcript and the charter judges rules
#               against it; lessons/entry.py answers nine of the third sweep's fifteen.
#               After the push: prewarm (a few rewritten Entry lines), then re-run Entry
#               once more -- then Basic.
...
#               09-13/14 ("is equal to", the receipt's 20 shown, the pause after the
#               value, one tape per teach beat, no cookies story). speechmap regenerated.
#               Prewarm owed: the changed lines.
...
#               rendered LIVE by ElevenLabs -- $17.35 of the $17.35 serving-side voice
#               bill, ~$4 per student-hour -- on a course whose every line is supposed
#               to be cached. The ledger counted them and could not name one: the usage
#               log keeps counts, not text (build ha's law), and nothing split the rows
#               by page. Now _tts_stream_response writes a `voice_miss` event on every
#               miss that will spend: name = the lane (speak / script / demo / ...),
#               detail = the length, whether the line is IN THE CLOSURE (a cache that
#               lost it, or a label that differs -- the real leak) or OUTSIDE it (a line
#               that varies, or a model's words), and a head of the text --
#               _voice_miss_head: 80 chars for a closure line, six words otherwise, the
#               student's own name replaced by [name] either way, never a student's
#               words (the speak endpoint only ever voices the tutor). store.usage_stats
#               gains tts_serve_by_mode; the night watch prints the misses by lane with
#               their lines; /api/admin/events' default feed adds voice_miss. PART 3lq.
...
#               lessons/prealgebra.py + lessons/algebra1.py: one beat each reads the
#               line it writes; speechmap.py regenerated. PART 3li.
...
#               PART 3le.
#   2026-09-11  APP_BUILD -> "2026-09-11vh-the-case-floor".
#               BUILD vh -- no change in this file beyond the stamp. tutor.py: the case
#               floor (repair_variable_case at the shipping door); nightwatch.py: the
#               stamp names code repairs. PART 3ld.
...
APP_BUILD = "2026-09-26yh-the-fourth-precalc-sweep"
...
def _voice_miss_head(text: str, code: str = "", in_closure: bool = False) -> str:
    ...
    try:
        ...
        head = t[:80] if in_closure else " ".join(t.split(" ")[:6])[:80]
        try:
            nm = ((store.get_account((code or "").strip()) or {}).get("name") or "").strip() \
                if (code and store.enabled()) else ""
        except Exception:  # noqa: BLE001
            nm = ""
        if nm and len(nm) >= 2:
            head = re.sub(re.escape(nm), "[name]", head, flags=re.I)
        return head
    except Exception:  # noqa: BLE001
        return ""
...
def _tts_stream_response(text: str, lead: int = 0, code: str = "", mode: str = "speak",
    ...
    # (vu) THE MISS GETS A FACE. A miss that will spend is written to system_events with
    # the lane, the size, whether the line belongs to the closure, and a head of the
    # text (see _voice_miss_head for what is kept out). A cached_only lane never spends
    # and is not a miss. Never raises: the voice is worth more than the tally.
    if not cached_only:
        try:
            _in_closure = text in _script_closure_texts()
            store.record_event("voice_miss", (mode or "speak")[:80],
                               f"{len(text)} chars, {'IN the closure' if _in_closure else 'outside the closure'}: "
                               f"{_voice_miss_head(text, code, _in_closure)}",
                               code or "")
        except Exception as _exc:  # noqa: BLE001
            print(f"[speak] miss event failed open: {_exc}")
