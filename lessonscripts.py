...
#               (ampl, pdeg, spin, imag, excl, expv, absv, rbet, lbet, sys3), Prob/Stat 9
#               (ntal, por, n68, inci, ccnt, zsco, indp, zval, cwid), Pre-Calc 9 (lsid,
#               lsol, pasc, lsub, ramp, refq, arsn both branches, negp, vasy), Algebra I 6
#               (yint, exmul, roots, foil, lny, outl), Geometry 4 (svol, mside, vert, twop),
#               Pre-Algebra 3 (prop both branches, bfac, asp), Basic 2 (rem, mtz both
#               branches), Diffeq 2 ASKS (cycl, estp -- no walk-back split would have
#               reached those). The opener stays the literal "Here it is, step by step: "
#               with its lower-case text, so every split lands at the first natural stop
#               AFTER the colon. No count moved; every wrong-path note still spoken.
...
#               their walk-backs in OTHER words ("Inside first: 3 plus 3 equals 6. Then 6
#               times 4 equals 24" against "inside the parentheses first -- 3 plus 3 equals
#               6. Then the times -- 6 times 4 equals 24"), and measured at that looser
#               bar the shape lived in 25 ops across six courses (rsol, evat, chev, refl,
#               notp, fpie, vprd, refq, wper, brng, ramp, lsid, lsub, htot, dcnt, iqrw,
#               spnt, sslp, sblw, resp, ccnt, wout, npop). Every one is a CREDIT LINE now
#               -- the answer and its one reason, marked "(xa)" -- and the walk-back keeps
#               the steps; PART 3mv pins the looser measurement. (2) asp's walk-back said
#               one long sentence after a miss; four short ones now. (3) dbf's number-line
#               walk-back board also writes "flip and times: 6 × 4/3 = 24/3 = 8", the
#               words its praise says. (4) uic offered "6 | 6 | 8" for 2 halves in 4
#               wholes (a×b−a met a+b): the third choice steps OVER when it would collide,
#               and the check demands three different options. Course lines 40,010 ->
#               40,005: the shorter praises share more text between lessons.
...
#               "some of it towers above that LINE" with no line drawn on the praise beat --
#               "some of the curve stands above that height and some falls below it". (3)
#               anti's board: "? x² came from it" had the relationship backwards -- "44x
#               came from ?x²"; the walk-back is short beats. (4) chan's praise and
#               walk-back: the inside's derivative is MULTIPLIED in, it does not "come down"
#               -- "two things are multiplied: the power comes down, and the inside's
#               derivative comes out to meet it". (5) prod's walk-back: 2c is "one piece of
#               the derivative, with the plus a forgotten", not "half the derivative". (6)
#               cfix's walk-back: a is "the plus number in the sloping piece", not "the
#               slope's own number". (7) trap's praise: "use that average speed for the c
#               seconds" (the train never holds it); the walk-back's two wrong answers are
#               two beats and "the halfway speed is the height of an equal-area rectangle".
#               (8) pgrw's ask names P and reads the board's dP/dt ("d P d t equals b P --
#               the rate is b times the amount"); the walk-back is short beats. (9) dfeq's
#               ask reads dV/dt before the board leans on it. (10) btwn's walk-back is short
#               beats and says "when one curve stays on top the whole way". (11) defi, (12)
#               mrat and (13) pwrc walk-backs are short beats (pwrc: "which is not the
#               power-rule move"). Authored: lessons/calculus.py.
...
def _rem_worked(p):
    ...
    spoken = (f"Here it is, step by step: {a} shared into groups of {b} fills "
              f"{_plural(q, 'group')} — {q} times {b} equals {q * b}. "
              f"Then {a} take away {q * b} leaves {r}. So {r} {'is' if r == 1 else 'are'} left over.")
    return (spoken, board)
...
def _dbf_worked(p):
    ...
    if n <= 8 and c * b <= 30:
        return (f"Here it is, step by step: dividing asks how many {_frac_words(a, b)} fit in {c}. "
                ...
                # (xa) the praise says "flip the fraction and times", so it is written too
                f'[[step eq="flip and times: {c} × {b}/{a} = {c * b}/{a} = {n}"]]')
...
def _asp_worked(p):
    ...
    if 100 % b == 0:
        k = 100 // b
        # (xa) short sentences after a miss
        return (f"Here it is, step by step: the bottom went from {b} to 100. That is timesed "
                f"by {k}. So the top is timesed by {k} too: {a} times {k} equals {ans}. "
                f"{a} out of {b} is {ans} percent.", pic)
...
def _lny_worked(p):
    a, b = p["a"], p["b"]
    return (f"Here it is, step by step: find {b} along the bottom and climb straight up to the line. "
...
def _vert_worked(p):
    ...
    return (f"Here it is, step by step: the angle next to {a} sits with it on one straight line. "
            f"The angles along a straight line make 180, so 180 take away {a} equals {180 - a}. The angle "
...
def _spin_worked(p):
    a = p["a"]
    return (f"Here it is, step by step: a full turn is 360, so {a} plus 360 equals {a + 360}. "
...
def _ampl_worked(p):
    a = p["a"]
    return (f"Here it is, step by step: the plain sine tops out at 1. The {a} out front "
...
def _vasy_worked(p):
    ...
    if a != b:
        ...
        return (f"Here it is, step by step: each factor dies at its own x. {a} zeroes the first "
                f"and {b} the second, and either one alone flattens the whole bottom. Look at "
...
def _lsol_worked(p):
    ...
    return (f"Here it is, step by step: the log counted {b} layers of {a}. So rebuild the number "
            f"by stacking them again — {a} multiplied out {b} times is {a ** b}. Not {a * b}: "
...
def _ramp_worked(p):
    a = p["a"]; h = a // 2
    return (f"Here it is, step by step: the sine of 30 degrees is one half, so the ramp climbs "
            f"half its length. Half of {a} is {h} feet. The {a} is how far you walk up the slope, "
...
def _lsub_worked(p):
    ...
    return (f"Here it is, step by step: nothing breaks at x equals {a}, so the value walks in "
            f"with x. {b} times {a} is {a * b}, plus {c} is {v}. For a line, the limit is "
...
def _lsid_worked(p):
    a, b, c = p["a"], p["b"], p["c"]
    if c == 0:
        spoken = (f"Here it is, step by step: from the left, every x you pass is below 6. "
                  f"So y reads {a} the whole way in — the limit from that side is {a}. The other "
                  f"side would say {b}, and the two do not have to agree.")
        pts, v, side = f"(3,{a}),(4,{a}),(5,{a})", a, "left"
    else:
        spoken = (f"Here it is, step by step: from the right, every x you pass is 6 or more. "
                  f"So y reads {b} the whole way in — the limit from that side is {b}. The other "
...
def _indp_worked(p):
    a, b, c = p["a"], p["b"], p["c"]
    return (f"Here it is, step by step: independent means the left-handers would look just like "
            f"the school. {a} percent, whether there are {b} of them or 200. They came in at "
...
def _ntal_worked(p):
    ...
    return (f"Here it is, step by step: 5 percent of {a} is {ends} out at the ends. The bell "
...
def _cwid_worked(p):
    a, b = p["a"], p["b"]; w = 2 * b
    return (f"Here it is, step by step: the range runs {b} below and {b} above. So it is {b} "
            f"twice — {w} points wide, from {a - b} to {a + b}. The margin {b} is one side of the "
...
def _cfix_worked(p):
    ...
    return (f"Here it is, step by step: walk the sloping piece right up to {c} and it arrives at "
            ...
            f"not fit, and {a} is only the plus number in the sloping piece.",
...
def _pwrc_worked(p):
    a, b = p["a"], p["b"]; t = a * b
    return (f"Here it is, step by step: the {a} comes down and meets the {b} standing there. "
            f"{a} times {b} is {t}, and the power drops to {a - 1}. So the derivative is {t} x "
            f"to the {a - 1}. {a + b} adds the two, which is not the power-rule move, and "
            f"{b} left the exponent up where it was.",
...
def _prod_worked(p):
    ...
    return (f"Here it is, step by step: feed {c} into 2 x plus {a} — {2 * c} plus {a} is {m}, the "
            ...
            f"{h} is the curve\'s height there, and {2 * c} is one piece of the derivative, "
            f"with the plus {a} forgotten.",
...
def _mrat_worked(p):
    ...
    return (f"Here it is, step by step: area is side squared. So its rate is 2 times the side, "
            f"times the side's rate: 2 times {a} times {b} is {g} square centimetres a second. "
...
def _anti_board(p):
    ...
    return (f'[[write text="derivative = {a}x"]]'
            ...
            f'[[step eq="{a}x came from ?x²"]]')


def _anti_worked(p):
    a = p["a"]; h = a // 2
    return (f"Here it is, step by step: forwards, {h} x squared drops its 2 down the front and "
            f"gives {a} x. So backwards, from {a} x, you halve: {a} over 2 is {h}. The power "
...
def _defi_worked(p):
    a, b = p["a"], p["b"]; d = a * b
    return (f"Here it is, step by step: the speed line sits at {a} for {b} seconds. The shape under "
            f"it is a rectangle {a} tall and {b} wide. {a} times {b} is {d}, and {d} metres "
            f"is how far the car went. The area MEANS the distance. {a + b} adds metres a "
            f"second to seconds, and {b} is the time you were told.",
...
def _btwn_worked(p):
    ...
    return (f"Here it is, step by step: the {b} is under the bottom curve. It is already inside the "
            f"{a} under the top one. So take it away: {a} take away {b} is {d}, the strip "
            f"between them. When one curve stays on top the whole way, it is top take away "
            f"bottom. {a + b} counts the lower region twice over, and {a} is the whole slab, "
            f"not the gap.",
...
def _pgrw_worked(p):
    ...
    return (f"Here it is, step by step: every one of the {a} bacteria contributes {b} a minute. So "
            f"the rate right now is {a} times {b} — {g} a minute. That is the height of the "
            f"line at P equals {a}. And it will not stay there: the growing feeds the growing. {a + b} "
...
OP_EXT = {
    ...
    "uic": {   # how many 1/a fit inside b wholes
        ...
        "choices": lambda p: [p["a"] * p["b"], p["a"] + p["b"],
                              p["a"] * p["b"] + p["a"] if p["a"] * p["b"] - p["a"] == p["a"] + p["b"]
                              else p["a"] * p["b"] - p["a"]],
        ...
        "check": lambda p: (2 <= p["a"] <= 10 and 2 <= p["b"] <= 8
                            ...
                            "the parts are 2-10 and the wholes 2-8, and the three "
                            "options differ -- at 2 and 2 a×b and a+b both give 4, and "
                            "at 2 and 4 the one-short option was a+b"),
    },
    ...
    "refl": {  # flip across the y line: x changes sign, y stays
        ...
        # (xa) a credit line; the walk-back has the steps
        "praise": lambda p: (f"Negative {p['a']}, {p['b']}: the x flips its "
                             f"sign across the y line, and the height stays."),
        "key": lambda p: p["a"],
    ...
    "notp": {  # the complement: chance a out of b -> no-chance is b - a out of b
        ...
        # (xa) a credit line; the walk-back has the steps
        "praise": lambda p: (f"{p['b'] - p['a']} out of {p['b']}: the rain "
                             f"chance and the no-rain chance share the whole "
                             f"{p['b']} between them."),
        "key": lambda p: p["b"] - p["a"],
    ...
    "rsol": {  # a / x = b: which x was fed? Rebuild, then divide.
        ...
        # (xa) a credit line; the walk-back has the steps
        "praise": lambda p: (f"x is {p['a'] // p['b']}. Check it forward: "
    ...
    "fpie": {  # piecewise: x + a below 5, b x at 5 or more -- run the RIGHT rule
        ...
        # (xa) a credit line; the walk-back has the steps
        "praise": lambda p: ((f"{p['c'] + p['a']} — the first rule, because "
                              f"{p['c']} is below 5.")
                             if p["c"] < 5 else
                             (f"{p['b'] * p['c']} — the second rule, because "
                              f"{p['c']} is 5 or more.")),
    ...
    "vprd": {  # Vieta: roots a and b -- the end number is their product
        ...
        # (xa) a credit line; the walk-back has the steps
        "praise": lambda p: (f"{p['a'] * p['b']} — the end number is the "
                             f"roots multiplied, {p['a']} times {p['b']}."),
        "key": lambda p: p["a"] * p["b"],
    ...
    "refq": {  # reference angle in the second quarter: the gap to 180
        ...
        # (xa) a credit line; the walk-back has the steps
        "praise": lambda p: (f"{180 - p['a']} degrees — the gap from "
                             f"{p['a']} to the flat line at 180."),
        "key": lambda p: p["a"],
    ...
    "wper": {  # period of sin(a x): divide 360 by the multiplier
        ...
        # (xa) a credit line; the walk-back has the steps
        "praise": lambda p: (f"{360 // p['a']} degrees — 360 divided by "
                             f"{p['a']}, because a faster wave's first repeat "
                             f"comes SOONER, not later."),
        "key": lambda p: p["a"],
    ...
    "ramp": {  # the 30-degree rise: height is HALF the ramp's length
        ...
        # (xa) a credit line; the walk-back has the steps
        "praise": lambda p: (f"{p['a'] // 2} feet — half of {p['a']}, "
                             f"because the sine of 30 degrees is one half."),
        "key": lambda p: p["a"],
    ...
    "lsub": {  # the friendly limit: walk x in, and the value walks in too
        ...
        # (xa) a credit line; the walk-back has the steps
        "praise": lambda p: (f"{p['a'] * p['b'] + p['c']} — for a line, the "
                             f"limit is where the line already is at x "
                             f"equals {p['a']}."),
        "key": lambda p: p["a"] * p["b"] + p["c"],
    ...
    "lsid": {  # one-sided limits: the two sides can disagree
        ...
        # (xa) a credit line; the walk-back has the steps
        "praise": lambda p: ((f"{p['a']} — from the left, y reads {p['a']} "
                              f"the whole way in.")
                             if p["c"] == 0 else
                             (f"{p['b']} — from the right, y reads {p['b']} "
                              f"the whole way in.")),
        "key": lambda p: p["a"] + p["b"],
    ...
    "dcnt": {  # counting a SLICE of a dot plot, with a dot on the line itself
        ...
        # (xa) a credit line; the walk-back has the steps
        "praise": lambda p: (f"{p['b']} — only the dots to the right of "
                             f"{p['a']}; the one on {p['a']} is not more than "
                             f"{p['a']}."),
        "key": lambda p: p["b"],
    ...
    "htot": {  # how many in all, from a histogram: add the printed bars
        ...
        # (xa) a credit line; the walk-back has the steps
        "praise": lambda p: (f"{p['a'] + p['b'] + p['c']} — every bar "
                             f"added, because each bar is a count of scores."),
        "key": lambda p: p["a"] + p["b"] + p["c"],
    ...
    "spnt": {  # read ONE dot off a scatterplot -- the axes are the trap
        ...
        # (xa) a credit line; the walk-back has the steps
        "praise": lambda p: (f"{_scat_at(p)} points — up from {p['a']} on "
                             f"the bottom to the dot, then across."),
        "key": lambda p: p["a"],
    ...
    "sslp": {  # the fit line's slope USED -- a rate, over several steps
        ...
        # (xa) a credit line; the walk-back has the steps
        "praise": lambda p: (f"{p['a'] * p['b']} points — {p['a']} each hour "
                             f"for {p['b']} hours, because the slope is a rate."),
        "key": lambda p: p["a"] * p["b"],
    ...
    "sblw": {  # the fit line runs THROUGH the cloud, so both sides are full
        ...
        # (xa) a credit line; the walk-back has the steps
        "praise": lambda p: (f"{p['a'] - p['b']} below — the {p['a']} dots "
                             f"minus the {p['b']} above, since every dot is "
                             f"on one side or the other."),
        "key": lambda p: p["a"],
    ...
    "resp": {  # the response rate: what percent actually came back
        ...
        # (xa) a credit line; the walk-back has the steps
        "praise": lambda p: (f"{100 * p['b'] // p['a']} percent — "
                             f"{p['b']} answered out of {p['a']} asked; that "
                             f"share is the response rate."),
    ...
    "ccnt": {  # the conditional rate itself, once the world has shrunk
        ...
        # (xa) a credit line; the walk-back has the steps
        "praise": lambda p: (f"{100 * p['a'] // (p['a'] + p['b'])} percent — "
                             f"{p['a']} of the {p['a'] + p['b']} girls, "
                             f"because the girls are the whole world now."),
    ...
    "wout": {  # without replacement: the second draw lives in a smaller world
        ...
        # (xa) a credit line; the walk-back has the steps
        "praise": lambda p: (f"{p['b'] - 1} — one marble stayed out, so "
                             f"the bag holds {p['b']} take away 1."),
        "key": lambda p: p["b"],
    ...
    "npop": {  # carry the range down onto real people
        ...
        # (xa) a credit line; the walk-back has the steps
        "praise": lambda p: (f"{(p['a'] - p['b']) * p['c'] // 100} — the "
                             f"low end, {p['a'] - p['b']} percent of "
                             f"{p['c']}, keeps the give-or-take."),
    ...
    "evat": {  # a derivative is a FUNCTION -- feed it an x
        ...
        # (xa) a credit line; the walk-back has the steps
        "praise": lambda p: (f"{2 * p['a'] * p['c']} — the slope right at "
                             f"x equals {p['c']}, from feeding {p['c']} to "
                             f"the derivative, {2 * p['a']} x."),
    ...
    "chev": {  # the chain rule, then fed a number
        ...
        # (xa) a credit line; the walk-back has the steps
        "praise": lambda p: (f"{2 * p['a'] * p['b']} — outside derivative "
                             f"times inside derivative, with the inside "
                             f"worth {p['b']} at x equals zero."),
    ...
    "cycl": {  # what a nonlinear system does that a linear one cannot
        ...
        "spoken": lambda p: (f"Nudge this model off the balance point and "
                             ...
                             f"round and round. In this tidy cycle the "
