# CHANGELOG -- topic.html  (notes rolled out of the file's header)

Moved out of `static/topic.html` on 2026-09-08 (build ui): every CHANGE NOTE dated before 2026-09-01 -- 65 entries, VERBATIM, in the order they sat in the file (newest first). The 12 notes from 2026-09-01 on stay at the top of `static/topic.html` itself, and new notes keep going there. Nothing below was edited; grep this file for a build letter or a date. When the header is rolled out again, the newer block is added ABOVE this one.
...
```text
    ...
    (de) 2026-08-10 -- DIFFEQ UNIT NAMES UPDATED to the CUPM mainstream restructure
    (build de): slope fields in unit 1, separable+linear merged in 2, qualitative
    analysis 3, numerical methods 4, nonhomogeneous+vibrations merged in 6, Laplace 7,
    linear systems 8, nonlinear systems 9. Series solutions dropped. Names MUST stay
    byte-identical to pedagogy.COURSE_PEDAGOGY["diffeq"]["unit_names"] and to
    session.html's CURRICULUM block -- ruletests checks the parity.
    ...
    2026-08-07  VOICE-FIRST CLASSROOM (Jim: "back to the conversational back-and-forth").
                ...
                (2) REMOVED the ⏸ Pause button and the Yes / No / I'm confused quick replies
                    (HTML, CSS, setPaused/setQuick, `paused` guards) -- the student just SAYS
                    it now. The 💡 New topic button STAYS (it's navigation).
                (3) 🧮 Math Keyboard retired (see math-keyboard.js): typing stays as the
                    fallback; the 📈 graph tool is unchanged.
  ...
  - 2026-08-03  ANALYTICS: added Plausible (privacy-friendly, cookieless) shared include in <head>. Pure add-on; nothing else changed.
    ...
    2026-07-30  MARKET PREP (paired with main.py 2026-07-30f-lockdown): /api/speak and
                ...
                &code=/?code=. Also renamed title to "MyTutor — Explore a Topic". Do no harm.
    2026-07-30  MIC CLEANUP + UI POLISH. (1) Removed the "🎙️" tap-and-say TOPIC-entry button, its CSS
                and recording JS, and the getUserMedia() warm-up in startTopic() -- no more microphone
                prompt (warm voice out / type in). (2) Answer boxes now read "Write your response here,
                then press Enter…". (3) 🧮 button renamed "Math Keyboard" via math-keyboard.js, which also
                shows a start-of-lesson reminder about it. (4) Left sidebar widened ~10% (280px -> 308px).
    ...
    2026-07-30  ANSWER-BAR CLEANUP (paired with math-keyboard.js). The redundant standalone "Send"
                ...
                the 🧮 and 📈 tools plus the Enter key all still send). The #chatInput placeholder now
                reads "Type your reply, then press Enter…" so a directly-typed reply is still
                sendable without a visible button. No send logic changed here. Static-only. Do no harm.
    ...
    2026-07-28  ADDED DIFFERENTIAL EQUATIONS to UNITS_BY_COURSE + a placeholder. Additive. Do no harm.
    2026-07-28  ADDED CALCULUS to UNITS_BY_COURSE + a placeholder. Additive. Do no harm.
    2026-07-28  ADDED PROBABILITY & STATISTICS to UNITS_BY_COURSE (its 9 unit names) + a placeholder.
                Additive; page reads ?course= and falls back. Do no harm.
    2026-07-28  ADDED TRIG / PRE-CALC to UNITS_BY_COURSE (its 9 unit names for the topic grid) + a
                course-appropriate placeholder. Additive; page reads ?course= and falls back. Do no harm.
    2026-07-28  COURSE-SCOPED CHECKS. The [[check]] POST now sends `course` so an end-of-unit check is
                filed under the right course's mastery (it was defaulting to Algebra I). Do no harm.
    2026-07-28  ADDED ALGEBRA II to UNITS_BY_COURSE (its 9 unit names for the topic grid) and a
                course-appropriate topic-box placeholder ("e.g. how logarithms work"). Additive; the
                page already reads ?course= and falls back to Algebra I, so nothing else changes. Do
                no harm.
    2026-07-28  COLUMN-MATH VISUAL [[column]]. New whiteboard visual for stacked, place-value /
                ...
                the student). Whole-number columns line up by place value too. Added .colmath CSS +
                splitNum/colOp/showColumn + a handleTags "column" case. Same visual added identically
                to session.html and practice.html. Static-only (no backend build change).
    2026-07-25  NAV BUTTONS + TYPE-UNDER-THE-BOARD. The left sidebar was missing the
                cross-mode nav the lesson page has, and typing was hidden behind a "Type
                instead" link. Fixed both: (1) added a <nav class="leftnav"> with My course /
                Practice a problem / Progress dashboard links (hrefs set from CODE in JS);
                (2) added a persistent .feedbar (chatInput + chatSend) UNDER the whiteboard
                that's always visible and posts through the same sendToTutor path (guarded by
                `busy`); (3) retired the old sidebar "Type instead" link via #typeToggle
                {display:none}. New CSS: .leftnav/.navbtn/.feedbar. No other logic changed.
